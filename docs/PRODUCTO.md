...
| Estado de conversación en **Redis con TTL de 30 minutos**, sin base de datos relacional | Empezó en memoria, y en producción se rompió: en serverless dos mensajes seguidos caen en instancias distintas y la persona tenía que contar su historia otra vez. El almacén es una interfaz con dos implementaciones, así que las pruebas y el simulador local siguen sin necesitar nada. La retención sigue siendo mínima —pasados 30 minutos no queda nada—, pero ahora el olvido lo garantiza un TTL y no el reinicio de un proceso. Ver `WHATSAPP.md` y `SEGURIDAD.md` regla 14. |
